...
w = I_J_e*q_dot;
q_dot_des = zeros(6,1);
q_ddot_des = zeros(6,1);
w_ref = I_J_e*q_dot_des;
w_dot_ref = I_J_e*q_ddot_des + I_dJ_e*q_dot_des;
y = pseudoInverseMat(I_J_e, lambda)*(w_dot_ref + kdMat*(w_ref-w) + kpMat*chi_err - I_dJ_e*q_dot);
tau = M_fun(q)*y + b_fun(q, q_dot) + g_fun(q);

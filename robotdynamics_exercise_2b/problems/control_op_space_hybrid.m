...
b = b_fun(q, dq);
g = g_fun(q);
lambda = pseudoInverseMat((I_Je/M)*I_Je', 0.01);
mu = lambda*(I_Je/M)*b - lambda*I_dJ_e*dq;
p = lambda*(I_Je/M)*g;
...
dchi_err = zeros(6,1) - I_Je*dq;
tau = I_Je'*(lambda*Sm*(kpMat*chi_err +kdMat*dchi_err) + Sf*I_F_E + mu + p); % TODO
